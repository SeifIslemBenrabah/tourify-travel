...
export const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-[#0c2638] border-t border-primary/10 text-white">
      {/* Newsletter Area */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 border-b border-primary/10">
            ...
            {isSubscribed ? (
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className={`flex-1 min-h-[48px] rounded-xl bg-white/5 border border-primary/10 px-4 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-accent ${isRTL ? "text-right" : "text-left"
                    }`}
                />
                <button
                  id="btn-subscribe-footer"
                  type="submit"
                  className="rounded-xl bg-accent text-xs font-bold uppercase tracking-wider px-6 py-3 sm:py-0 transition-colors shadow-lg shadow-accent/20 cursor-pointer w-full sm:w-auto"
                >
                  {t.subscribeButton}
                </button>
              </form>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        <div className={`flex  flex-col ${isRTL ? "text-right space-y-4 items-end" : "text-left items-start"}`}>
          ...
          <p className={`text-xs text-white/50 leading-relaxed font-sans w-full ${isRTL ? "items-end" : "items-start"}`}>
            {t.footerInfo}
          </p>
          <div className={`flex gap-3 pt-2 ${isRTL ? "w-full items-end" : "w-full items-start"}`}>
            <a
              ...
              className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-white hover:text-white/60 transition-all border border-primary/10"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-white hover:text-white transition-all border border-primary/10"
            >
        ...
        <div className={`space-y-4 ${isRTL ? "text-right" : "text-left"}`}>
          ...
          <div className="relative h-32 rounded-2xl overflow-hidden bg-white border border-primary/10 flex flex-col justify-end p-3 text-[10px] text-white/60 font-mono">
            {/* Grid Line Visuals */}
            ...
            <div className="absolute inset-0 flex flex-col justify-between p-1 opacity-25">
              <div className="border-b border-primary/10 w-full" />
              <div className="border-b border-primary/10 w-full" />
              <div className="border-b border-primary/10 w-full" />
            </div>
      ...
      {/* Underbar Copyright */}
      <div className="bg-primary-dark py-6 border-t border-primary/10 text-center text-[11px] text-white/60">
