  ...
  return (
    <div className="space-y-16 pb-20 text-white bg-primary-dark font-sans">
      ...
      <section className="relative h-[60vh] flex items-end overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-t from-primary-dark via-primary-dark/40 to-black/30 z-10" />
        <img
          src={dest.image}
          alt={dest.name}
        ...
        <div className="relative z-15 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 space-y-4 animate-in fade-in slide-in-from-bottom-5 duration-450">
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-accent hover:text-white transition-colors cursor-pointer group mb-6 bg-primary-dark/60 backdrop-blur-sm pl-3 pr-4 py-2 rounded-full border border-white/5"
          >
          ...
          <div className="space-y-2 text-left">
            ...
            <h1 className="font-display font-black text-3xl sm:text-5xl md:text-6xl text-white uppercase tracking-wider max-w-4xl drop-shadow-lg">
              {dest.name}
            </h1>
            
            <div className="flex flex-wrap items-center gap-6 text-xs text-white/80 pt-1">
              ...
              <div className="flex items-center gap-1 bg-white/5 border border-white/10 px-2.5 py-1 rounded-full">
                ...
                <span className="text-white/45">({dest.reviewsCount} reviews)</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-12 text-left">
            ...
            <div className="space-y-4">
              <h3 className="font-display font-bold text-xl uppercase tracking-wider border-b border-white/5 pb-2 text-white">
                {currentLanguage === "ar" ? "عن الرحلة الاستكشافية" : "THE ESCAPADE BLUEPRINT"}
              </h3>
              <p className="text-sm text-white/70 leading-relaxed font-sans font-medium whitespace-pre-wrap">
                {dest.description}
              </p>
            </div>
            ...
            {dest.images && dest.images.length > 0 && (
              <div className="space-y-4">
                <h4 className="font-display font-semibold text-sm uppercase tracking-wider text-white/60">
                  {currentLanguage === "ar" ? "ألبوم صور الرحلة" : "VISUAL GALLERY SLIDE"}
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-3 rounded-2xl overflow-hidden h-72 border border-white/10 shadow-lg">
                    <img
                      src={activeImg}
                      alt="Active visual"
                  ...
                  {dest.images.map((imgUrl, idx) => (
                    <button
                      ...
                      className={`rounded-xl overflow-hidden h-20 border transition-all cursor-pointer ${
                        activeImg === imgUrl ? "border-accent ring-2 ring-accent/25" : "border-white/10 hover:border-white/30"
                      }`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-primary/45 p-6 rounded-3xl border border-white/5 shadow-inner">
              
              {/* Included services */}
              <div className="space-y-3">
                <h5 className="font-display font-bold text-sm text-[#FFFFFF] uppercase tracking-wider border-b border-white/10 pb-2 flex items-center gap-1.5">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 relative border-l border-white/10 ml-4 pl-6">
                {dest.itinerary.map((it) => {
                  ...
                  return (
                    <div key={it.day} className="relative group">
                      ...
                      <span className={`absolute -left-[35px] top-1.5 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black tracking-wider transition-colors z-10 ${
                        isOpen ? "bg-accent text-white" : "bg-primary text-white/50 border border-white/10"
                      }`}>
                        {it.day}
                      </span>

                      <div className="rounded-2xl bg-primary border border-white/5 overflow-hidden text-white shadow-md">
                        <button
                        ...
                        >
                          ...
                          <ChevronRight size={16} className={`transform transition-transform text-white/50 ${isOpen ? "rotate-90 text-accent" : ""}`} weight="thin" />
                        </button>
                        {isOpen && (
                          <div className="p-4 bg-primary-dark/40 border-t border-white/5 text-xs text-white/70 leading-relaxed font-sans">
                            {it.description}
                          </div>
                        )}
                      </div>
            ...
            {/* Reviews list */}
            <div className="space-y-6">
              <h3 className="font-display font-bold text-xl uppercase tracking-wider border-b border-white/10 pb-2 text-white">
                {currentLanguage === "ar" ? "أراء المغامرين والضيوف" : "PILGRIMS & EXPLORERS FEEDBACK"}
              </h3>

              <div className="space-y-4">
                {dest.reviews.map((rev) => (
                  <div key={rev.id} className="p-5 rounded-2xl bg-white/5 border border-white/5 space-y-3">
                    ...
                    <p className="text-xs text-white/70 leading-relaxed italic font-sans">
                      "{rev.comment}"
                    </p>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            <div className="p-6 rounded-3xl bg-primary border border-white/10 shadow-2xl space-y-6 sticky top-24">
              
              {/* Action buttons (Share) */}
              <div className="flex justify-center items-center bg-white/5 px-4 py-2 text-xs font-semibold rounded-xl border border-white/5">
                <button
                  id="det-share-btn"
                  onClick={handleShare}
                  className="flex items-center gap-1.5 text-white/80 hover:text-white cursor-pointer"
                >
              ...
              <div className={`space-y-4 ${isRTL ? "text-right" : "text-left"}`}>
                ...
                <button
                  ...
                  className="w-full h-12 rounded-2xl bg-accent hover:bg-accent-hover text-white text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-accent/20 cursor-pointer flex items-center justify-center gap-2"
                >
                  <span>{t.bookNow}</span>
                  <ChevronRight size={14} className={isRTL ? "rotate-180" : ""} weight="thin" />
                </button>

                <div className="flex gap-2 p-3 rounded-xl bg-white/5 border border-white/5 items-start text-[10px] text-white/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {relatedList.map((item) => (
            <div
              ...
              className="group cursor-pointer rounded-2xl overflow-hidden bg-primary border border-white/5 hover:border-accent/15 transition-all shadow-md flex flex-col space-y-4 p-4"
            >
              <div className="h-44 rounded-xl overflow-hidden relative">
                ...
                <span className="absolute bottom-2 left-2 px-2.5 py-1 text-[9px] font-bold bg-primary-dark/80 rounded">
                  {item.category === "local" ? t.local : t.international}
                </span>
              </div>
              <div className="space-y-1">
                <h5 className="font-display font-semibold text-sm uppercase text-white tracking-wide truncate group-hover:text-accent transition-colors">
                  {item.name}
                </h5>
