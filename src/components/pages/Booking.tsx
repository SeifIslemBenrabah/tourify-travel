...
export const Booking: React.FC = () => {
  ...
  return (
    <div className={`space-y-16 pb-20 text-primary bg-white font-sans ${isRTL ? "text-right" : "text-left"}`} dir={isRTL ? "rtl" : "ltr"}>
          ...
          {[1, 2, 3, 4, 5].map((s) => {
            ...
            return (
              <div key={s} className="relative z-10 flex flex-col items-center">
                <span className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-black border transition-all ${current ? "bg-accent-white border-accent" : "bg-white text-primary/60 border-primary/10"
                  }`}>
                  {s}
                </span>
                <span className={`hidden md:block text-[9px] uppercase font-bold tracking-wider mt-2 ${current ? "text-accent" : active ? "text-primary" : "text-primary/60"
                  }`}>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-12 p-8 rounded-3xl bg-[#f0f4f8] border border-primary/10 shadow-xl space-y-6 min-h-[400px]">
            ...
            {step === 1 && (
              <div className={`space-y-6 animate-in fade-in duration-200 ${isRTL ? "text-right" : "text-left"}`}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-primary/10 pb-3">
                  <div>
                    <h3 className="font-display font-bold text-lg text-primary uppercase tracking-wider flex items-center gap-2">
                  ...
                  <div className="flex flex-col gap-1 w-full sm:w-auto shrink-0">
                    <label className="text-[9px] uppercase font-bold text-primary/60">{currentLanguage === "ar" ? "تغيير وجهة الرحلة" : "Switch Sanctuary"}</label>
                    <select
                      id="book-main-dest-selector"
                      value={selectedDestId}
                      onChange={(e) => setSelectedDestId(e.target.value)}
                      className="bg-white font-sans text-xs font-bold text-primary rounded-xl border border-primary/10 p-2.5 focus:outline-none focus:ring-1 focus:ring-accent min-w-[200px] cursor-pointer"
                    >
                      {destinations.map(d => (
                ...
                <div className="relative p-5 rounded-2xl bg-white/40 border border-primary/10 flex flex-col md:flex-row gap-5 items-center">
                  <img src={activeDest.image} className="w-full md:w-32 h-20 rounded-xl object-cover shrink-0" />
                  <div className="flex-1 min-w-0 text-center md:text-left">
                    <h4 className="font-display font-semibold text-primary text-sm uppercase tracking-wide truncate">
                      {activeDest.name}
                    </h4>
                    <div className="flex items-center justify-center md:justify-start gap-1 text-[10px] text-primary/60 font-mono mt-1">
                      <MapPin size={11} className="text-accent" weight="thin" />
                      <span>{activeDest.country}</span>
                    </div>
                    <p className="text-[11px] text-primary/60 line-clamp-2 mt-2 font-sans leading-relaxed">
                      {activeDest.description}
                    </p>
                  </div>
                  <div className="shrink-0 flex md:flex-col justify-between items-center md:items-end w-full md:w-auto border-t md:border-t-0 md:border-l border-primary/10 pt-3 md:pt-0 md:pl-5 gap-2">
                    <div className="text-left md:text-right">
                      <span className="block text-[8px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "أيام الرحلة" : "Duration"}</span>
                      <span className="text-xs font-bold text-primary font-mono bg-white/5 px-2 py-0.5 rounded border border-primary/10">{activeDest.duration}</span>
                    </div>
                    <div className="text-right">
                      <span className="block text-[8px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "السعر الأساسي" : "Base Price"}</span>
                ...
                <div className="space-y-3">
                  <h5 className="text-[10px] font-black tracking-wider uppercase text-primary/60">
                    ...
                    {availableSchedules.map((schedule) => {
                      ...
                      return (
                        <label
                          ...
                          className={`block p-4 rounded-xl border transition-all cursor-pointer flex items-center gap-3 ${isSelected ? "border-accent bg-accent/10" : "border-primary/10 bg-white/30 hover:bg-white/60"
                            }`}
                        >
                          <input
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="block text-[11px] font-bold text-primary font-mono">{schedule.label}</span>
                            <div className="flex justify-between items-center mt-2 text-[10px] text-primary/60">
                              <span className="font-mono bg-white/5 px-1.5 py-0.5 rounded text-primary/60">{schedule.daysNights}</span>
                ...
                <div className="flex justify-end pt-4 border-t border-primary/10">
                  <button
                    ...
                    className="py-3 px-8 rounded-2xl bg-accent-white text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1 cursor-pointer"
                  >
            ...
            {step === 2 && (
              <div className={`space-y-6 animate-in fade-in duration-200 ${isRTL ? "text-right" : "text-left"}`}>
                <div className="border-b border-primary/10 pb-2">
                  ...
                  <p className="text-xs text-primary/60">{currentLanguage === "ar" ? "حدد عدد الأفراد وفئة الضيافة المفضلة لحزم الإقامة" : "Define specifications of travelers and premium suite bracket status"}</p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="p-3.5 rounded-xl bg-accent/5 border border-accent/15 max-w-sm flex items-center gap-3">
                    ...
                    <div>
                      <span className="block text-[8px] uppercase tracking-wider text-primary/60">{currentLanguage === "ar" ? "موعد المغادرة المحدد" : "Selected Departure Date"}</span>
                      {availableSchedules.find(s => s.date === travelDate)?.label ? (
                        <span className="text-xs font-bold text-primary font-mono">{availableSchedules.find(s => s.date === travelDate)?.label}</span>
                      ) : (
                        <span className="text-xs font-bold text-primary font-mono">{travelDate}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className={`space-y-1.5 flex flex-col ${isRTL ? "items-start" : "items-start"}`}>
                    <label className={`block text-[10px] text-primary/60 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{currentLanguage === "ar" ? "عدد المعتمرين / المسافرين الكلي" : "Total Traveler Specifications"}</label>
                    <div className="flex items-center gap-3">
                      <button
                        id="btn-dec-travelers"
                        type="button"
                        onClick={() => setTravelersCount(prev => Math.max(1, prev - 1))}
                        className="w-10 h-10 rounded-xl bg-white border border-primary/10 flex items-center justify-center font-bold text-primary hover:border-accent"
                      >
                        -
                      </button>
                  ...
                  <div className="space-y-2">
                    <label className={`block text-[10px] text-primary/60 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{currentLanguage === "ar" ? "فئة الإقامة والضيافة" : "Accommodation Bracket Option"}</label>
                    <div className="grid grid-cols-3 gap-3 bg-white p-1 rounded-2xl border border-primary/10 text-xs font-bold text-center">
                      {[
                        ...
                        <button
                          ...
                          className={`p-3 rounded-xl transition-all cursor-pointer flex flex-col items-center gap-0.5 
                            ${packageClass === cls.key ? "bg-accent-white/5 hover:text-primary hover:bg-white/5" : ""}`}>
                          <span>{cls.title}</span>
                          <span className="text-[9px] text-primary/60 block font-normal font-mono">{cls.desc}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex justify-between pt-6 border-t border-primary/10">
                  <button
                    ...
                    className="py-3 px-6 rounded-2xl border border-primary/10 hover:border-primary/10 text-primary text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1 cursor-pointer"
                  >
            ...
            {step === 3 && (
              <div className={`space-y-6 animate-in fade-in duration-200 ${isRTL ? "text-right" : "text-left"}`}>
                <div className="border-b border-primary/10 pb-2">
                  ...
                  <p className="text-xs text-primary/60">{currentLanguage === "ar" ? "برجاء كتابة وثائق السفر وجواز السفر لملاءمة تصاريح نسك أو تأشيرات الوجهة" : "Specify passenger details and valid travel documents"}</p>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className={`block text-[10px] text-primary/60 mb-1.5 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{currentLanguage === "ar" ? "الاسم الكامل (مطابق للجواز)" : "Lead Traveler Full Name"} *</label>
                    <input
                      id="book-lead-name"
                      type="text"
                      required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className={`block text-[10px] text-primary/60 mb-1.5 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{currentLanguage === "ar" ? "رقم جواز السفر" : "Passport Reference Number"} *</label>
                      <input
                        ...
                        className="w-full rounded-xl bg-white border border-primary/10 px-4 py-3 text-xs text-primary focus:outline-none focus:ring-1 focus:ring-accent font-mono"
                      />
                    </div>
                    <div>
                      <label className={`block text-[10px] text-primary/60 mb-1.5 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{t.phoneLabel}</label>
                      <input
                        id="book-lead-phone"
                        type="tel"
                        value={leadPhone}
                  ...
                  <div>
                    <label className={`block text-[10px] text-primary/60 mb-1.5 font-bold uppercase ${isRTL ? "text-right" : "text-left"}`}>{t.emailLabel} *</label>
                    <input
                      id="book-lead-email"
                      type="email"
                      required
            ...
            {step === 4 && (
              <div className={`space-y-6 animate-in fade-in duration-200 ${isRTL ? "text-right" : "text-left"}`}>
                ...
                <div className="p-6 rounded-2xl bg-white/50 border border-primary/10 space-y-5 text-xs font-sans">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 leading-relaxed">
                    {/* Destination */}
                    <div className="space-y-1">
                      <span className="block text-[10px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "الوجهة المطلوبة" : "Target Destination"}</span>
                      <strong className="text-primary text-sm uppercase block font-display">{activeDest.name} ({activeDest.country})</strong>
                      <span className="text-[10px] text-primary/60 bg-white/5 px-2 py-0.5 rounded border border-primary/10 inline-block font-mono mt-0.5">{activeDest.duration}</span>
                    </div>

                    {/* Target dates */}
                    <div className="space-y-1">
                      <span className="block text-[10px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "تاريخ السفر والمغادرة" : "Departure Date"}</span>
                      <strong className="text-primary text-sm block font-mono">
                    ...
                    {/* Traveler amount */}
                    <div className="space-y-1">
                      <span className="block text-[10px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "عدد المجموعات والدرجة" : "Guests & Standard"}</span>
                    ...
                    {/* Final Pricing */}
                    <div className="space-y-1">
                      <span className="block text-[10px] text-primary/60 uppercase tracking-wider">{currentLanguage === "ar" ? "إجمالي القيمة المقدرة للرحلة" : "Total Booking Expense (CCP)"}</span>
                  ...
                  <div className="border-t border-primary/10 pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono text-primary/60">
                      <div>
                        <span className="block text-[9px] text-primary/60 font-sans uppercase mb-0.5">{currentLanguage === "ar" ? "الاسم الكامل (مطابق للجواز)" : "Lead Traveler Full Name"}</span>
                        <span className="text-primary font-sans font-semibold text-xs">{leadName}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-primary/60 font-sans uppercase mb-0.5">{currentLanguage === "ar" ? "رقم جواز السفر" : "Passport Reference Reference"}</span>
                        <span className="text-primary font-semibold text-xs">{leadPassport}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-primary/60 font-sans uppercase mb-0.5">{t.emailLabel}</span>
                        <span className="text-primary text-xs">{leadEmail}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-primary/60 font-sans uppercase mb-0.5">{t.phoneLabel}</span>
                        <span className="text-primary text-xs">{leadPhone}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-accent-white/">
                  ...
                  <div>
                    ...
                    <p className="font-sans text-[11px] text-primary/60">
                      {currentLanguage === "ar"
                ...
                <div className="flex justify-between pt-6 border-t border-primary/10">
                  ...
                  <button
                    id="step4-submit-btn"
                    onClick={handleNextStep}
                    className="py-3 px-8 rounded-2xl bg-accent-white text-xs font-black uppercase tracking-widest transition-all flex items-center gap-1.5 cursor-pointer shadow-lg shadow-accent/25 animate-pulse"
                  >
                    {isRTL && <ChevronLeft size={14} weight="thin" />}
            ...
            {step === 5 && (
              <div className="space-y-8 animate-in zoom-in-95 duration-200 text-center py-6 font-sans">
                ...
                <div className="space-y-3">
                  ...
                  <h3 className="font-display font-black text-2xl uppercase tracking-wider text-primary">
                ...
                <div className={`max-w-md mx-auto rounded-3xl overflow-hidden bg-white/85 border border-primary/10 p-6 space-y-4 ${isRTL ? "text-right" : "text-left"} shadow-2xl relative`} dir={isRTL ? "rtl" : "ltr"}>

                  {/* Header */}
                  <div className={`flex justify-between items-center border-b border-primary/10 pb-3 ${isRTL ? "flex-row" : "flex-row-reverse"}`}>
                    <div className="font-display font-black text-xs text-primary flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "الوجهة المعنية" : "Destination"}</span>
                      <span className="font-display font-semibold text-primary uppercase truncate block">{activeDest.name}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "المسافر الرئيسي" : "Lead Pilgrim"}</span>
                      <span className="font-semibold text-primary truncate block">{leadName}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "تاريخ الإقلاع والبدء" : "take-off Date"}</span>
                      <span className="font-semibold text-primary font-mono">{travelDate}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "الفئة والدرجة" : "Class Tier"}</span>
                    ...
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "مدة الإقامة" : "duration"}</span>
                      <span className="font-semibold text-primary font-mono">{activeDest.duration}</span>
                    </div>
                    <div>
                      <span className="block text-[8px] text-primary/60 uppercase">{currentLanguage === "ar" ? "عدد الأفراد" : "Travelers"}</span>
                      <span className="font-semibold text-primary">{travelersCount} {currentLanguage === "ar" ? "مسافرين" : "Person(s)"}</span>
                    </div>
                  </div>

                  {/* Pricing footer */}
                  <div className="pt-4 border-t border-dashed border-primary/10 flex items-center justify-between text-xs font-semibold">
                    <span className="text-primary/60 uppercase">{currentLanguage === "ar" ? "قيمة تثبيت تذكرة الرحلة" : "Verified holding cost"}</span>
                  ...
                  <div className="flex flex-col items-center pt-2 gap-2 border-t border-primary/10">
                    <QrCode size={45} className="text-primary" weight="thin" />
                    <span className="text-[9px] font-mono text-primary/60 tracking-[0.2em]">{recordedId}-{activeDest.id.toUpperCase().slice(0, 4)}</span>
                  </div>

                  {/* Ticket notch cutouts */}
                  <div className="absolute -left-3 top-2/3 -translate-y-1/2 w-6 h-6 rounded-full bg-[#f0f4f8]" />
                  <div className="absolute -right-3 top-2/3 -translate-y-1/2 w-6 h-6 rounded-full bg-[#f0f4f8]" />
                </div>
                ...
                <div className="pt-4 max-w-sm mx-auto">
                  <button
                    ...
                    className="w-full py-3 rounded-2xl bg-accent-white text-xs font-black uppercase tracking-wider transition-all cursor-pointer"
                  >
